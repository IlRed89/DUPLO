...
/**
 * Rinomina un file tramite modale a tema + IPC `rename-file`.
 * Aggiorna subito path, nome visibile e handler «apri cartella».
 *
 * @param {Object} group
...
async function askRenameFile(group, fileIndex) {
  ...
  logToMain('info', '[Rename] apertura dialog per "' + file.path + '" (nome="' + currentName + '")');
  ...
  try {
    ...
    logToMain('info', '[Rename] ok: visibile "' + file.name + '" path="' + newPath + '"');
    renderResults();
  } catch (err) {
    logToMain('error', '[Rename] eccezione: ' + err.message);
    await uiAlert(t('alert.renameFail', { error: err.message }), 'error');
  }
}
