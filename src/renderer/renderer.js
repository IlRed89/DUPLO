...
function initFolderDropZone() {
  ...
  function collectDroppedFiles(dataTransfer) {
    ...
    try {
      if (dataTransfer && dataTransfer.files) Array.from(dataTransfer.files).forEach(add);
    } catch (_err) { /* ignore */ }
    try {
  ...
  function onDrop(event) {
    try {
      allowCopyDrop(event);
      try { event.stopPropagation(); } catch (_err) { /* ignore */ }
      var files = collectDroppedFiles(event.dataTransfer);
      var stashed = consumeStashedPaths();
      dragCounter = 0;
      hideOverlay();
...
async function handleFolderDrop(files, stashedPaths) {
  try {
    ...
    function pushPath(nativePath) {
      if (!nativePath || seen[nativePath]) return;
    ...
    try { console.info('[DUPLO drop] elementi rilasciati:', fileList.length, 'path:', paths.length); } catch (_err) { /* ignore */ }
    ...
    for (i = 0; i < paths.length; i += 1) {
      var droppedPath = paths[i];
      logToMain('debug', 'Drop path nativo: ' + droppedPath);
      try { console.info('[DUPLO drop] path', droppedPath); } catch (_err) { /* ignore */ }
      var result = null;
      try {
