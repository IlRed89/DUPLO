...
/**
 * Etichette localizzate unite da " + " (es. "Stessa Dimensione + Stessa Estensione").
 *
 * @param {string[]} list
 * @returns {string}
 */
...
function buildReasonSection(reason, groups, criteria) {
  ...
  desc.textContent = t('reason.applied', { list: titleText });
...
function buildDuplicateCard(group, groupIdx) {
  ...
  if (applied.length > 0) {
    ...
    criteriaEl.className = 'group-criteria';
    criteriaEl.textContent = t('reason.applied', { list: formatCriteriaLabels(applied) });
